...
const HANDLERS: Array<{ method: string; pattern: RegExp; handler: Handler }> = [
  ...
  { method: "POST", pattern: /^\/findings\/([^/]+)\/remediation\/analyze$/, handler: (_b, _q, m) => enterpriseDemo.remediationIntelligence(m[1]) },
  { method: "GET", pattern: /^\/findings\/([^/]+)\/remediation$/, handler: (_b, _q, m) => enterpriseDemo.findingRemediations(m[1]) },
  { method: "GET", pattern: /^\/remediation\/([^/]+)$/, handler: (_b, _q, m) => need(enterpriseDemo.remediationById(m[1]), "Remediation not found") },
  { method: "POST", pattern: /^\/remediation\/([^/]+)\/validate-plan$/, handler: (_b, _q, m) => enterpriseDemo.validateRemediation(m[1]) },
