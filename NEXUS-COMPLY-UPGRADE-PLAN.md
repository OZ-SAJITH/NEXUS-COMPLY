...
- `apps/web/src/components/assets/*` (AssetCard, AssetStatusChip, DiscoveryWizard,
  ...
  - Shipped so far (PHASE 4 follow-up): `PostureCards` (shared byRegion/byFramework
    ...
    long-unused `GET /assets/:id/impact` surface on `AssetDetailPage`. Remaining
    entries (AssetCard, AssetStatusChip, DiscoveryWizard) are covered inline by
    the shipped pages above.
