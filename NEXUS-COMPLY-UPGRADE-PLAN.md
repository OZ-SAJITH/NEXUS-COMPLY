...
- **Tests**: new `apps/api/test/ai-remediation.test.ts` (19 tests) — catalog units (valid structural plan for TLS-001, INSUFFICIENT_EVIDENCE with no evidence, baseline disclaimer, correct action set, rollback AVAILABLE, malformed-plan rejection, confidence 0.30 base, evidence-grounded certainty, deterministic change risk, REVIEW_REQUIRED without rollback) + service integration (enriched RemediationRecord, baseline disclaimer, rollback, evidence with hash, version increments reusing the same record, status stays PLANNED, audit event detail, list returns enriched, risk matches catalog). Totals: **API 150/150 (13 files), Web 44/44 (2 files)**; `apps/api` tsc clean; `apps/web` tsc + vite build clean. Web: `enterpriseParity.test.ts` PHASE 6 describe (store analyze/version/audit + demoApi route mirror incl. 404 + full closed-loop verify) and `enterpriseRender.test.tsx` PHASE 6 jsdom probe (panel sections render; RemediationsPage shows the generate button and renders the inline panel after expanding the card).
