...
- `apps/web/src/components/assets/*` (AssetCard, AssetStatusChip, DiscoveryWizard,
  ...
  - Shipped so far (PHASE 4 follow-up): `PostureCards` (shared byRegion/byFramework
    ...
    per-domain bars) extracted from `DashboardPage`. `ImpactGraphView` (tiered
...
- **Tests**: API 113/113 across 11 files (new `apps/api/test/governance.test.ts` — 16 tests: framework catalog, policy profiles, deterministic selection, applicable controls, full trace, exception lifecycle, byRegion/byFramework summary incl. PASS-included regions; new `apps/api/test/connector-gate.test.ts` — 8 tests: SIMULATED/PRODUCTION classification, authorized-action admit, unauthorized-action deny, production denied by default policy / without allowlisting / admitted once enabled + allowlisted). Web 35/35 across 2 files (parity: 5 PHASE 4 parity tests — framework catalog + 404, policies + deterministic selection, asset governance trace + per-framework + applicable controls + evaluate, exception lifecycle via REST surface **with governance audit events asserted**, compliance summary byRegion/byFramework with PASS controls counted; render probes: 4 PHASE 4 jsdom probes — GovernancePage hub incl. auto-loading decision trace + selected-asset `AssetCard` link, AssetDetailPage Adaptive Governance panel + impact cascade, DashboardPage posture panel, EnterpriseAuditPage governance-exception chips — plus 2 `components/assets` probes for `AssetStatusChip` tones and `AssetCard` navigation). Builds clean.
...
### PHASE 5 — deterministic risk prioritization + asset impact graph (shipped)
- **Network exposure model** (`packages/enterprise-catalog/src/risk.ts`): `networkExposureOfAsset(asset)` derives a `NetworkExposure` class (INTERNAL / RESTRICTED / PARTNER / DMZ / INTERNET_FACING) deterministically from zone/tier/tags (internet-facing|payment-border tags → INTERNET_FACING; DMZ|CLOUD → DMZ; MGMT|TIER_3 → RESTRICTED; partner|extranet → PARTNER; else INTERNAL) with per-class `EXPOSURE_WEIGHT`. Legacy risk scores and weights are preserved exactly (incl. `pci` tag still rendering 1.0 exposure), so every control's risk-factor decomposition is unchanged at equal input.
- **Bounded impact graph** (`packages/enterprise-catalog/src/impact.ts` rewrite): `buildImpactGraph(asset, allAssets, {maxDepth})` builds the asset→service→application→database→business cascade with a hard `MAX_GRAPH_NODES=120` cap and cycle-safe BFS; nodes are enriched with assetType / region / tier / environment / riskScore / riskBand / complianceStatus plus a synthetic `kind: "internet"` edge node. `findingBlastRadius` (depth 4) returns affectedAssetIds/count, criticalAssetsAffected, affectedServices, regionsAffected (always includes the root region, sorted) and crossRegion. `impactScoreFor` is fully deterministic and capped 0–100: breadth ≤ 25 (count×8), critical downstream ≤ 20 (count×10), root criticality ≤ 15 (CRITICAL 15 / HIGH 12 / MEDIUM 8 / LOW 5), production ≤ 15, cross-region ≤ 10, service diversity ≤ 15 (services×5).
- **Potential exposure path** (`exposurePathForAsset`): reverse-traces edge devices (FW/ROUTER/PROXY/SWITCH/LB) from the asset to `Internet`, then lists the asset and its **top critical/high downstream** dependents — bounded (maxDepth 8) and explicitly labeled *"Potential exposure path … not evidence of compromise"* (spec §32: never "confirmed attack path").
- **Finding risk context** (`buildFindingRiskContext` + `FindingRiskInput`): per-finding `FindingRiskContext` carrying riskScore/riskLevel (band), severity, exposure (derived class, not a stored field), asset criticality + name, evidenceConfidence, impactScore, `blastRadius`, a 5-factor `contributors` breakdown (severity/exposure/criticality/importance/exploitability points, each equal to the matching `explainableRisk` factor) and a human `explanation` string; falls back to `explainableRisk` when no prior riskExplanation exists.
- **API surface** (`apps/api/src/routes/enterprise.ts` + service): `GET /assets/:id/impact` (existing route now wired) + alias `GET /assets/:id/graph`, `GET /findings/:id/impact` (risk context with evidence confidence from `evidenceForFinding`), `GET /findings/:id/exposure-path`. Shared types (`packages/shared-types`) gained `NetworkExposure`, expanded `ImpactNode` and `FindingBlastRadius` / `FindingRiskContext` / `ExposurePathHop` / `AssetExposurePath`.
- **Frontend**: `ImpactGraphView` gains an `internet` node kind (Globe) and region/risk chips; new `RiskContextPanel` ("Potential impact / blast radius" — risk score/level, exposure, asset criticality, evidence confidence, impact score, blast-radius chips, per-contributor bars, "Why this risk?" prose + View-impact-graph anchor) and `ExposurePathList` (hop chips with protocol/port, "Potential exposure path" label + disclaimer); `RiskExplainCard` shows the panel per finding when context is available; `AssetDetailPage` batches contexts and renders the exposure path under the cascade (id=`impact` anchor).
- **Demo parity**: `enterpriseStore.ts` + `demoApi.ts` mirror `assetGraph` / `findingImpact` / `findingExposurePath` exactly (same deterministic catalog functions on persisted state).
- **Tests**: new `apps/api/test/risk-context.test.ts` (18 tests) — exposure class derivation, determinism, criticality/exposure/production ordering, blast-radius counts + regions + cross-region, isolated-asset no blast, cyclic graphs terminate (bounded), exposure path Internet→edge→asset, contributors match `explainableRisk` factors, service integration round-trip. Totals: **API 131/131 (12 files), Web 36/36 (2 files)**; `apps/api` tsc clean; `apps/web` tsc + vite build clean.
- Live API probe (port 4000): `GET /api/assets/ast-api-gateway-01/impact` and `/graph` → 200; `GET /api/findings/<TLS-001-id>/impact` → risk 98 CRITICAL, exposure INTERNET_FACING, impact 89, blast 3 assets / 3 critical / regions IND, contributors sev+35 exp+30 crit+15, evidence confidence 0.98; `GET /api/findings/ast-api-gateway-01/exposure-path` → `Internet → FW-DMZ-01 → API-GATEWAY-01 → DB-CUSTOMER-CORE → WEB-PAYMENTS`, depth 5, "potential path, not a confirmed attack path" disclaimer.
- Final report: `docs/PHASE5_FINAL_REPORT.md` (19 sections, incl. §17 manual demo steps and §13 live-probe results).

### PHASE 6 — AI remediation intelligence engine (shipped)
- **Deterministic intelligence engine** (`packages/enterprise-catalog/src/intelligence.ts`): `buildRemediationIntelligence(input)` produces `AiRemediationIntelligence` from evidence + asset + finding + `FindingRiskContext` — action catalog with describe/validate/rollback per `RemediationActionType`, `intentFromActionType`, `topPriorityActions(actionType)` (e.g. TLS-001 → SET_TLS_MIN_VERSION + ENFORCE_STRONG_CIPHERS), `changeRiskFor` (deterministic score + reason from action impact / exposure / environment / blast radius / rollback availability / destructive-in-prod / connector authorization; `REVIEW_REQUIRED` when rollback is unavailable or the action is destructive in prod), `confidenceFromEvidence` (0.30 no evidence → up to 0.98 evidence-grounded), `evidenceForFinding→IntelligenceEvidenceInput` mapping with `verifyEvidenceRecord` re-derived SHA-256, `validationStepsFor` (BEFORE/CHANGE/AFTER phases + expected evidence), rollback plan cloning from the action def, `buildReasoningChain` prose, `validateAiRemediationPlanShape` (structural validator), and `BASELINE_REMEDIATION_LABEL = "Baseline remediation guidance"`. Any plan created without a live provider is labeled baseline, never presented as an AI verdict.
- **Service orchestration** (`apps/api/src/services/enterprise/remediationService.ts`): `analyzeRemediationIntelligence` loads the finding + asset + all assets, resolves evidence (finding evidence, else asset evidence via `evidenceForAsset`), computes the `FindingRiskContext` through the catalog, builds the plan deterministically, and **stores intelligence + plan versions inline on the `RemediationRecord`** (`intelligence`, `planVersions[]` sliced to the latest 20). Versioning: `version = max(previous intelligence version, planVersions.length) + 1`; every analyze appends a `RemediationPlanVersion` snapshot (version/status/approval NONE/execution NOT_EXECUTED). `requiresApproval: true` is forced for every generated plan — the AI never short-circuits the human gate. Provider mode: only `AI_PROVIDER==="live"` calls the live AI worker endpoint — now implemented in `apps/ai-service` (`routes/remediation_ai.py`, `schemas/remediation.py`, `interpreters/remediation_mock.py`; POST `{AI_SERVICE_URL}/api/ai/remediation-plan`, 6 s abort) with the same evidence-only pydantic contract and a prose-only `RemediationPlanResponse` (structure + approval stay backend-owned; any interpreter exception falls back to the deterministic mock planner). An **honesty guard** labels the plan by the provider that actually produced content: `provider` is `"live"` only when applied live prose enrichment exists (summary/rootCause/potentialImpact override fields), otherwise `"mock"` — a failed or absent live call is never logged as live. Any failure or non-live mode falls back to the deterministic engine (source `"deterministic"`, provider `"mock"`, baseline disclaimer). The plan is shape-validated via `validateAiRemediationPlanShape` before it is persisted (malformed → ApiError 500). A `AI_REMEDIATION_INTELLIGENCE_GENERATED` audit event (entityType `remediation`, source `ai`, detail = intelligenceId/version/source/provider/requiresApproval) is logged by actor SYSTEM_REVIEWER.
- **API surface** (`apps/api/src/routes/enterprise.ts`): `POST /findings/:id/remediation/analyze` (returns the enriched `RemediationRecord`), `GET /findings/:id/remediation` (list looking up by finding), `GET /remediation/:id` (detail), `POST /remediation/:id/validate-plan` (thin alias of the existing plan validation — validates the current `intelligence` version). The returned record is the same `RemediationRecord` the existing closed-loop UI already drives, so Validate → request approval → Approve → Execute → Verify → Rollback is unchanged.
- **Frontend** (`apps/web/src/components/assets/AiRemediationIntelligencePanel.tsx`): "AI Remediation Intelligence" panel rendered inline in the expanded `RemediationWorkflow` card whenever `rem.intelligence` exists — header (version badge, source badge "Baseline remediation guidance" vs "AI-assisted", exported `ChangeRiskPill`, confidence), AI-unavailable notice when source ≠ ai, Summary, Root cause + certainty badge (EVIDENCE_GROUNDED/INSUFFICIENT_EVIDENCE), Evidence used (verified chip + hash short), Recommended actions (change-risk pill, "approval required" badge, target/configArea/expectedState/reason), Pre-checks, Potential impact + blast chips, Validation plan (BEFORE/CHANGE/AFTER), Rollback plan + status, Expected result, Change risk, connector capability (vendor-aware) + Unavailable list, collapsible "Why this remediation?", disclaimer. `RemediationWorkflow` gains a "Generate AI plan" / "Regenerate AI plan" button (PLANNED/VALIDATION_FAILED → `remediationIntelligence(findingId)`), `RiskExplainCard` gains an "AI remediation plan" button (`onRemediationIntelligence`), and `AssetDetailPage` wires `remediateIntelligence` to analyze then navigate to the workflow. Demo store + demoApi mirror the four routes (`enterpriseStore.remediationIntelligence`, list/detail/validate-plan handlers). `apps/web/src/services/api.ts` adds `remediationIntelligence`.
- **Tenets enforced**: evidence stays a *consideration* (never a promise of AI ground truth — plans built deterministically from stored evidence with re-derived hashes); the plan only proposes a *change* → validated → human-approved → executed by the orchestrator with the production policy gate intact; live-provider output is flattened to prose/overrides over the deterministic structure, keeping the shape validator in control; provider labeling is honest — the recorded `provider` reflects who actually produced content, so failures/absence are never labeled `live`.
- **Tests**: new `apps/api/test/ai-remediation.test.ts` (22 tests) — catalog units (valid structural plan for TLS-001, INSUFFICIENT_EVIDENCE with no evidence, baseline disclaimer, correct action set, rollback AVAILABLE, malformed-plan rejection, confidence 0.30 base, evidence-grounded certainty, DB-001 database-encryption plan for DB-CUSTOMER-CORE producing ENABLE_DB_ENCRYPTION with pre-checks/validation/rollback and no fabricated vendor commands, deterministic change risk, REVIEW_REQUIRED without rollback) + service integration (enriched RemediationRecord, baseline disclaimer, rollback, evidence with hash, version increments reusing the same record, status stays PLANNED, audit event detail, list returns enriched, risk matches catalog) + live-provider enrichment & honest fallback (a stubbed live worker applies prose overrides → `source=ai`/`provider=live` with the §29 system-prompt safety contract sent verbatim over a structured evidence-only context; a failing call (503) → `source=deterministic`/`provider=mock` with the baseline disclaimer). Totals: **API 153/153 (13 files), Web 44/44 (2 files)**; `apps/api` tsc clean; `apps/web` tsc + vite build clean. Web: `enterpriseParity.test.ts` PHASE 6 describe (store analyze/version/audit + demoApi route mirror incl. 404 + full closed-loop verify) and `enterpriseRender.test.tsx` PHASE 6 jsdom probe (panel sections render; RemediationsPage shows the generate button and renders the inline panel after expanding the card).
- Live API probe (port 4919): `POST /api/findings/<TLS-001-id>/remediation/analyze` → 200 `status=PLANNED`, `intelligence v1 source=deterministic provider=mock requiresApproval=true rollback=AVAILABLE certainty=EVIDENCE_GROUNDED changeRisk=HIGH`, 1 evidence used, 2 recommended actions; `GET /api/findings/<id>/remediation` → 1 record with intelligence; `GET /api/remediation/<id>` → same record; `POST /api/remediation/<id>/validate-plan` → `VALIDATED`; unknown finding analyze → 404.
- Browser verification against the production bundle (`vite preview:4173`, demo mode): `logs/phase6-probe.mjs` — 17/17 interactive checks (expand hero card → "Generate AI plan" → inline panel: baseline-guidance source badge, root cause, evidence used, recommended actions with approval-required badge, pre-checks/validation/rollback/expected-result sections, change risk + confidence, collapsible "Why this remediation?" reasoning chain with the AI-never-executes line → "Regenerate AI plan" bumps v1→v2 never overwriting → audit trail "AI REMEDIATION INTELLIGENCE GENERATED"), 0 console errors; screenshots `logs/phase6-ai-panel.png` + `logs/phase6-why-expanded.png`.
- Final report: `docs/PHASE6_FINAL_REPORT.md` (20 sections, incl. §36 manual demo steps and live probe results); `docs/DEMO_SCRIPT.md` gains the PHASE 6 walkthrough (generate → baseline-guidance label → approval gate → regeneration v2).
